...
$base_sql = "
    ...
    WHERE sr.student_id = ? AND sr.status = 'accepted' AND sr.student_deleted = 0
";
if ($id > 0) {
...
$msg = 'Masha’Allah! Your recitation of <strong>' . $surah . '</strong> (' . $verses . ') was accepted.';
if ($rec['rating'] !== null && trim((string)$rec['rating']) !== '') {
    $msg .= ' Rating: <strong>' . htmlspecialchars($rec['rating']) . '</strong>.';
}
$teacher = trim((string)$rec['feedback']);
if ($teacher !== '') {
    $msg .= '<br><br>' . nl2br(htmlspecialchars($teacher));
}
$msg .= '<br><br>Keep up the great work!';

ui_message_page('success', 'Recitation Accepted', $msg, 'feedback.php', 'View Full Feedback');
