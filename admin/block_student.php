...
$stmt = $conn->prepare("SELECT blocked FROM users WHERE id=? AND role='student'");
$stmt->bind_param("i", $student_id);
$stmt->execute();
$current = $stmt->get_result()->fetch_assoc();

$new_status = $current['blocked'] ? 0 : 1; // toggle
...
if ($is_ajax) {
    echo $new_status ? 'Student blocked.' : 'Student unblocked.';
    exit;
}
